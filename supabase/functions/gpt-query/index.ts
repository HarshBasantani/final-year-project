...
serve(async (req) => {
  ...
  try {
    ...
    if (!query || typeof query !== "string" || !query.trim()) {
      return new Response(
        JSON.stringify({ error: "Missing or empty 'query' field", code: 400 }),
    ...
    const selectedModel = modelMap[model] || "openai/gpt-5-mini";
    ...
    if (!aiResponse.ok) {
      ...
      console.error("GPT gateway error:", aiResponse.status, text);
      return new Response(
    ...
    console.error("gpt-query error:", e);
    return new Response(
